...
| 11 | **Comps analysis spreadsheet (standalone)** | Standalone Google Sheets + Apps Script tool for importing sold property comps from any source (Redfin, Zillow, LandInsights, PropStream, MLS, county assessor, ATTOM, etc.). Requires: sale price, lot size/acreage, lat/lng, sale date. Pipeline: (1) column mapping from source format, (2) time adjustment to normalize historical pricing to today's dollars — primary method is auto-calculating appreciation/depreciation rate from a trend line fit to the dataset itself (most accurate for raw land); fallback is USDA NASS land value index (state-level, land-specific, more relevant than FHFA which tracks house prices); manual override field always available for counties where user has direct market knowledge; option to skip adjustment entirely for thin datasets — (3) Isolation Forest to flag statistical outliers missed in manual review, (4) DBSCAN on lat/lng to identify geographic pricing pockets, (5) IDW to generate continuous price surface data, (6) Jenks Natural Breaks within each cluster to find natural acreage tiers, (7) KNN validation to confirm tier pricing against nearest comps. Outputs structured pricing model dataset for upload to LandValuator. | ~4-6 hrs |
| 12 | **Algorithm-driven pricing in LandValuator** | Upload comps analysis output from #11 into the app. Netlify function computes IDW price surface. Heat map overlay on Mapbox map (toggleable color gradient showing pricing across county). When syncing LandInsights sheet: algorithm computes retail PPA × acreage per property based on lat/lng + acreage → writes computed retail price directly to sheet (bypasses zone letter system). Manual zones remain for precision overrides (subdivisions, lake communities, etc.) and override algorithm pricing for any property inside them. Zones are no longer required for general county pricing. | ~6-8 hrs |
